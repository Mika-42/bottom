...
void ui_scroll(ui_t *ui, const int dx, const size_t selected) {
	...
	getmaxyx(stdscr, terminal_height, terminal_width);
	int max_scroll_x = 0;
	if ((int)ui_pad_columns > terminal_width) {
		max_scroll_x = ui_pad_columns - terminal_width;
	}
	ui->ui_scroll_x += dx;
...
void ui_select(ui_t *ui, user_selection_t *s) {
	...
	if (s->indices.size != 0) {
		if (s->selected > 0) {
			mvwchgat(ui->pad, s->selected - 1, 0, -1, A_NORMAL, 0, nullptr);
		}
		mvwchgat(ui->pad, s->selected, 0, -1, A_REVERSE, 0, nullptr);
		if (s->selected < s->indices.size) {
			mvwchgat(ui->pad, s->selected + 1, 0, -1, A_NORMAL, 0, nullptr);
		}
	}
